...
# Check and create redistribute interfaces on a remote device
# Usage: ensure_redistribute_interfaces <host> <auth_credential> <auth_type> <user> <interfaces>
# Parameters:
#   interfaces - Space-separated list of interface names (e.g., "lo1 dum-ex")
# Note: No IP is assigned to interfaces - they are created as empty dummy interfaces
ensure_redistribute_interfaces() {
    local host="$1"
    local auth_credential="$2"
    local auth_type="${3:-key}"
    local user="${4:-ubuntu}"
    local interfaces="${5:-lo1}"

    log_info "Checking redistribute interfaces on $host: $interfaces"

    local ssh_opts="-o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10"

    # Build the interface check and creation script
    # This script:
    # 1. Checks if each interface exists
    # 2. Creates missing interfaces as dummy type (no IP assignment)
    # 3. Makes them permanent via netplan
...
# Function to create dummy interface temporarily (no IP assigned)
create_dummy_interface() {
    local iface=\"\$1\"
    
    echo \"Creating dummy interface: \$iface\"
    sudo ip link add \"\$iface\" type dummy 2>/dev/null || true
    sudo ip link set \"\$iface\" up
}

# Function to make interface permanent via netplan (no IP assigned)
make_interface_permanent() {
    local iface=\"\$1\"
    
    local netplan_file=\"/etc/netplan/99-redistribute-interfaces.yaml\"
    
    # Check if netplan file exists
    if [[ ! -f \"\$netplan_file\" ]]; then
        echo \"Creating netplan configuration for redistribute interfaces...\"
        sudo tee \"\$netplan_file\" > /dev/null << 'NETPLAN_HEADER'
# Redistribute interfaces for BGP
# Generated by prepare-frr.sh
# Note: No IPs assigned - IPs are managed by FRR/applications
network:
  version: 2
  dummy-devices:
NETPLAN_HEADER
    ...
    # Check if interface already in netplan
    if grep -q \"^[[:space:]]*\$iface:\" \"\$netplan_file\" 2>/dev/null; then
        echo \"  Interface \$iface already in netplan\"
        return 0
    fi
    
    # Add interface to netplan (no addresses)
    echo \"  Adding \$iface to netplan for persistence\"
    sudo tee -a \"\$netplan_file\" > /dev/null << NETPLAN_IFACE
    \$iface:
      addresses: []
NETPLAN_IFACE
}
...
for iface in \$INTERFACES; do
    ...
    if interface_exists \"\$iface\"; then
        ...
        # Still ensure it's in netplan for persistence
        make_interface_permanent \"\$iface\"
    else
        echo \"  Interface \$iface does not exist, creating...\"
        create_dummy_interface \"\$iface\"
        make_interface_permanent \"\$iface\"
    fi
done
...
sudo netplan apply 2>/dev/null || echo \"Warning: netplan apply had issues (non-fatal)\"

# Verify interfaces
echo \"\"
echo \"Redistribute interfaces status:\"
for iface in \$INTERFACES; do
    if interface_exists \"\$iface\"; then
        echo \"  ✓ \$iface: exists (no IP - managed by FRR/applications)\"
    else
        echo \"  ✗ \$iface: MISSING\"
    fi
done
"
    ...
    if [[ "$auth_type" == "key" ]]; then
        ssh $ssh_opts -i "$auth_credential" "${user}@${host}" "$interface_script" 2>&1
        result=$?
    else
        sshpass -p "$auth_credential" ssh $ssh_opts "${user}@${host}" "$interface_script" 2>&1
        result=$?
    fi

    if [[ $result -eq 0 ]]; then
        log_success "Redistribute interfaces configured on $host"
    else
        log_warn "Some issues with redistribute interfaces on $host (may be non-fatal)"
    fi

    return 0  # Don't fail on interface issues
}

# Check if FRR is already running on a remote device
# Usage: check_frr_not_running <host> <auth_credential> <auth_type> <user>
# Returns: 0 if FRR is NOT running (safe to install), 1 if FRR IS running (abort)
check_frr_not_running() {
    local host="$1"
    local auth_credential="$2"
    local auth_type="${3:-key}"
    local user="${4:-ubuntu}"
...
# Also check if frr package is installed
if dpkg -l frr 2>/dev/null | grep -q "^ii"; then
    echo "FRR_INSTALLED"
    exit 0
fi

echo "FRR_NOT_PRESENT"
exit 0
'

    local result=""
    if [[ "$auth_type" == "key" ]]; then
        result=$(ssh $ssh_opts -i "$auth_credential" "${user}@${host}" "$check_script" 2>/dev/null)
    else
        result=$(sshpass -p "$auth_credential" ssh $ssh_opts "${user}@${host}" "$check_script" 2>/dev/null)
    fi

    case "$result" in
        "FRR_RUNNING")
            log_error "FRR is already running on $host"
            log_error "Use configure-frr.sh to update configuration, or stop FRR first"
            return 1
            ;;
        "FRR_INSTALLED")
            log_error "FRR is already installed on $host (but not running)"
            log_error "If you want to reinstall, first remove it: apt remove frr frr-pythontools"
            return 1
            ;;
        "FRR_NOT_PRESENT")
            log_info "FRR not present on $host, proceeding with installation..."
            return 0
            ;;
        *)
            log_warn "Could not determine FRR status on $host, proceeding anyway..."
            return 0
            ;;
    esac
}
...
# Usage: install_frr_on_device <host> <auth_credential> <auth_type> <user> [redistribute_interfaces]
install_frr_on_device() {
    local host="$1"
    local auth_credential="$2"
    local auth_type="${3:-key}"
    local user="${4:-ubuntu}"
    local redistribute_interfaces="${5:-lo1}"

    log_info "Installing FRR on $host..."

    # Check if FRR is already running - abort if so
    if ! check_frr_not_running "$host" "$auth_credential" "$auth_type" "$user"; then
        return 1
    fi
...
'
    ...
    if [[ $result -eq 0 ]]; then
        ...
        # Now ensure redistribute interfaces exist
        if [[ -n "$redistribute_interfaces" ]]; then
            ensure_redistribute_interfaces "$host" "$auth_credential" "$auth_type" "$user" "$redistribute_interfaces"
        fi
    else
...
prepare_device_from_config() {
    ...
    # Get redistribute interfaces from config (if specified)
    local redistribute_interfaces=$(yq eval ".frr_config.devices[$device_index].redistribute_interfaces | join(\" \")" "$config_file" 2>/dev/null || echo "")
    if [[ "$redistribute_interfaces" == "null" || -z "$redistribute_interfaces" ]]; then
        # Default for hypervisors
        local device_type=$(yq eval ".frr_config.devices[$device_index].type // \"hypervisor\"" "$config_file")
        if [[ "$device_type" == "hypervisor" ]]; then
            redistribute_interfaces="lo1 dum-ex"
        else
            redistribute_interfaces="lo1"
        fi
    fi

    # Install FRR and configure redistribute interfaces (no IP assignment)
    install_frr_on_device "$host" "$auth_credential" "$auth_type" "$ssh_user" "$redistribute_interfaces"
}
...
prepare_from_vdc_topology() {
    ...
    for device_name in $devices; do
        ...
        # Get device type and configure redistribute interfaces accordingly
        local device_type=$(echo "$device_info" | yq eval '.type' - 2>/dev/null)

        # Set redistribute interfaces based on device type
        local redistribute_interfaces="lo1"
        if [[ "$device_type" == "hypervisor" ]]; then
            redistribute_interfaces="lo1 dum-ex"
        fi

        # Check for custom redistribute_interfaces in VDC config
        local custom_interfaces=$(echo "$device_info" | yq eval '.redistribute_interfaces | join(" ")' - 2>/dev/null || echo "")
        if [[ -n "$custom_interfaces" && "$custom_interfaces" != "null" ]]; then
            redistribute_interfaces="$custom_interfaces"
        fi

        # Install FRR and configure redistribute interfaces (no IP assignment)
        install_frr_on_device "$mgmt_ip" "$auth_credential" "$auth_type" "$SSH_USER" "$redistribute_interfaces"
    done
}

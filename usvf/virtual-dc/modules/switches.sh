...
create_sonic_cloud_init() {
    ...
    log_info "Creating cloud-init configuration for $full_vm_name (Ubuntu)..."

    # Create user-data
    # NOTE: FRR configuration is NOT included here - it will be configured
    # via the FRR module (frr/configure-frr.sh) after VMs are SSH-accessible
...
EOF
  ...
  - |
    ...
    FRRouting:  Will be configured via FRR module

    Quick Commands (after FRR setup):
      - vtysh               # Enter FRR CLI
...
create_sonic_vm() {
    ...
    for i in $(seq 0 $((iface_count - 1))); do
        local iface_name=$(yq eval ".switches.$tier[$index].data_interfaces[$i].name" "$config_file")
